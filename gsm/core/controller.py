...
class GameController(Named, Transactionable, Savable):
	...
	def _reset(self, player, seed=None):
		...
		if self._pre_setup_complete is not None:
			info = None
			try:
				info = containerify(yaml.load(open(self._pre_setup_complete, 'r'), Loader=yaml.SafeLoader))
			except:
				pass
	...
	def _load_config(self):
		...
		for name, path in self.config_files.items():
			config[name] = containerify(yaml.load(open(path, 'r'), Loader=yaml.SafeLoader))
